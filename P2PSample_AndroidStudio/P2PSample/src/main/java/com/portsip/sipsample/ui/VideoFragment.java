...
public class VideoFragment extends Fragment implements View.OnClickListener ,PortMessageReceiver.BroadcastListener{
	...
	private void stopVideo(PortSipSdk portSipLib)
	{
		...
		{
			portSipLib.setConferenceVideoWindow(null);
		}
		if (cur != null
				&& !cur.isIdle()
	...
	public void updateVideo(PortSipSdk portSipLib)
	{
		if (application.mConference)
		{
			...
			portSipLib.setConferenceVideoWindow(remoteRenderScreen);
			return;
		}
				...
				&& cur.hasVideo)
		{
			...
			portSipLib.displayLocalVideo(true, application.mUseFrontCamera);
